...
-jugarPartida() agrupa la generación del mazo, la mezcla, el reparto y llama a jugarRonda() para arrancar el juego; se ejecuta al cargar la página y cada vez que se hace clic en el botón #botonNuevaRonda. También resetea enDesempate a false en cada partida nueva, para que un desempate anterior no afecte a la siguiente partida.
-mostrarCartas() vacía y vuelve a dibujar las 4 cartas en pantalla junto con los botones de descarte del Jugador 1; se llama desde jugarPartida(), elJugadorDescarta() y turnoReinaRoja(), para que la imagen se actualice cada vez que cambian las cartas. Usando la variable esperandoDescarte, arma cada botón con el atributo disabled cuando no le toca elegir al jugador, y sin ese atributo (activo) cuando sí le toca descartar.
-jugarRonda() comprueba si el juego ya terminó (algún jugador con 3 puntos) o si ambos llegaron a 3 puntos en la misma ronda, en cuyo caso activa enDesempate = true y deja que el juego siga jugando en lugar de cortar. También comprueba si el mazo se agotó (recicla descartes) y si el Jugador 1 tiene par: si tiene par, suma el punto, renueva sus 2 cartas (las descarta y roba 2 nuevas del mazo) y, si está en desempate, chequea si ya sacó ventaja de puntos para declarar la victoria; si no tiene par, activa esperandoDescarte, actualiza la pantalla y conecta 2 botones (uno por carta) con addEventListener, deteniéndose con return hasta que el jugador haga clic.
-elJugadorDescarta(cartaATirar) se ejecuta al hacer clic en un botón: descarta la carta elegida, roba una nueva del mazoMezclado, comprueba si ahora hay par (y, si está en desempate, si ya ganó), desactiva esperandoDescarte y llama a turnoReinaRoja().
-turnoReinaRoja() juega el turno automático de la computadora: si tiene par, suma el punto, renueva sus 2 cartas y comprueba la victoria por desempate igual que del lado del jugador; si no tiene par, descarta y roba una carta. Al final llama de nuevo a jugarRonda() para continuar con la siguiente ronda, hasta que algún jugador gane.
