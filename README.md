...
-Dentro del while, antes de comprobar el par de cada jugador, se revisa si mazoMezclado se quedó sin cartas (mazoMezclado.length === 0); si es así, se recicla el array descartes: se mezclan sus cartas al azar (mismo mecanismo que se uso para mezclar el mazo al principio) y se agregan de vuelta a mazoMezclado con push(), para que el juego pueda seguir robando.
-Cuando el while termina (porque algún jugador llegó a 3 puntos), se compara puntosJugador1 === 3 para mostrar por consola quién ganó la partida.
-Todo el proceso (generar mazo, mezclar, declarar variables, repartir, el while y el anuncio del ganador) se agrupo en una función jugarPartida(), para poder ejecutarlo varias veces sin repetir código.
-Se agregó un botón #botonNuevaRonda: su addEventListener llama a jugarPartida() en cada clic, reiniciando el juego completo. Antes de mostrar las cartas repartidas, se vacía mesaDeJuego.innerHTML = "" para que las imágenes no se acumulen entre partidas sucesivas.